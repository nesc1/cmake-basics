...
# Programs
if(WIN32)
    #in windows clang-cl and lld-link fail to link because does not include clang_rt.profile-x86_64.lib
    find_library(CLANG_PROFILE_LIB 
        NAMES clang_rt.profile-x86_64 clang_rt.profile-i386   
    )
    #find grcov
    find_program(GRCOV_PATH grcov)
else()
...
if(CODE_COVERAGE)
    ...
    if("${CMAKE_C_COMPILER_ID}" MATCHES "(Apple)?[Cc]lang" OR "${CMAKE_CXX_COMPILER_ID}" MATCHES "(Apple)?[Cc]lang")
        ...
        if(WIN32)
            if(NOT CLANG_PROFILE_LIB)
                message(WARNING "clang profile library not found...link my not work")
            endif()
            #test grcov
            if(NOT GRCOV_PATH)
                message(FATAL_ERROR "grcov not found! Aborting.")
            endif()
        else()
    ...
    if(CODE_COVERAGE)
        ...
        if("${CMAKE_C_COMPILER_ID}" MATCHES "(Apple)?[Cc]lang" OR "${CMAKE_CXX_COMPILER_ID}" MATCHES "(Apple)?[Cc]lang")
            if(WIN32)
                ...
                # if win32 need to force the link of profile lib
                target_link_libraries(${TARGET_NAME} PRIVATE ${CLANG_PROFILE_LIB})    
            else()
        ...
        if(target_type STREQUAL "EXECUTABLE")
            if("${CMAKE_C_COMPILER_ID}" MATCHES "(Apple)?[Cc]lang" OR "${CMAKE_CXX_COMPILER_ID}" MATCHES "(Apple)?[Cc]lang")
                ...
                add_custom_target(ccov-show-${TARGET_NAME}
                    COMMAND ${LLVM_COV_PATH} show $<TARGET_FILE:${TARGET_NAME}> -instr-profile=${TARGET_NAME}.profdata -show-line-counts-or-regions ${EXCLUDE_REGEX}
                    DEPENDS ccov-processing-${TARGET_NAME}
                )

                add_custom_target(ccov-report-${TARGET_NAME}
                    COMMAND ${LLVM_COV_PATH} report $<TARGET_FILE:${TARGET_NAME}> -instr-profile=${TARGET_NAME}.profdata ${EXCLUDE_REGEX}
                    DEPENDS ccov-processing-${TARGET_NAME}
                )

                add_custom_target(ccov-${TARGET_NAME}
                    COMMAND ${LLVM_COV_PATH} show $<TARGET_FILE:${TARGET_NAME}> -instr-profile=${TARGET_NAME}.profdata -show-line-counts-or-regions -output-dir=${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/${TARGET_NAME} -format="html" ${EXCLUDE_REGEX}
                    DEPENDS ccov-processing-${TARGET_NAME}
                )
...
function(add_code_coverage)
    if("${CMAKE_C_COMPILER_ID}" MATCHES "(Apple)?[Cc]lang" OR "${CMAKE_CXX_COMPILER_ID}" MATCHES "(Apple)?[Cc]lang")
        if(WIN32)
            ...
            # if win32 need to force the link of profile lib
            link_libraries(${CLANG_PROFILE_LIB})    
        else()
    ...
    if(CODE_COVERAGE)
        if("${CMAKE_C_COMPILER_ID}" MATCHES "(Apple)?[Cc]lang" OR "${CMAKE_CXX_COMPILER_ID}" MATCHES "(Apple)?[Cc]lang")
            ...
            add_custom_target(ccov-all-report
                COMMAND ${LLVM_COV_PATH} report `cat ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list` -instr-profile=${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/all-merged.profdata ${EXCLUDE_REGEX}
                DEPENDS ccov-all-processing
            )

            add_custom_target(ccov-all
                COMMAND ${LLVM_COV_PATH} show `cat ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list` -instr-profile=${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/all-merged.profdata -show-line-counts-or-regions -output-dir=${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/all-merged -format="html" ${EXCLUDE_REGEX}
                DEPENDS ccov-all-processing
            )
